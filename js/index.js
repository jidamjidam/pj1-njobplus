window.addEventListener("load", function () {
    ...
    const pictures = document.querySelectorAll("section.visual picture");

    let currentIndex = 0;
    const totalImages = pictures.length;
    const duration = 4000; // 3초마다 이미지 전환

    // 첫 번째 picture 활성화
    pictures[currentIndex].classList.add('active');

    function showNextImage() {
        // 현재 활성화된 picture에서 active 클래스를 제거 (fade out)
        pictures[currentIndex].classList.remove('active');

        // 다음 인덱스 계산 (마지막 이미지일 경우 처음으로 돌아감)
        currentIndex = (currentIndex + 1) % totalImages;

        // 다음 picture 활성화 (fade in)
        pictures[currentIndex].classList.add('active');
    }
    ...
    if (!isMobile) {
        ...
        let Slide_Carousel_02 = Slide_Carousel(".new_class", 4000, 2, 2, 1, "slide", false, false);
        let Slide_Carousel_03 = Slide_Carousel(".review", 4000, 4, 3, 1, "slide", false, false);
    ...
    let Slide_Carousel_01 = Slide_Carousel(".top_class", 4000, 1, 1, 1, "slide", false, false);
